...
can tell whether one is usable for a run (see `loader._archive_sources`).
"""
import dataclasses
import json
...
class SmokeEntry:
    ...
    full archive it was built from, at Hub commit `revision`. `error` is the read
    error that stopped the build, if one did.
    """
    clips_per_split: int
    clips: dict[str, int]
    source_sha256: str
